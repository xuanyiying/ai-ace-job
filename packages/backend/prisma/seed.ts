...
Result: Reduced deployment time from 2 hours to 15 minutes, improved system scalability, enabled independent team deployments, and reduced mean time to recovery (MTTR) from 30 minutes to 5 minutes.`,
  ...
  for (const question of commonInterviewQuestions) {
    try {
      ...
      // Since vectorDocument model doesn't exist, we'll skip creation entirely
      console.log(`⏭️  Skipping creation (no vectorDocument model): ${question.content.substring(0, 50)}...`);
      skipped++;
    } catch (error) {
      console.error(
        `❌ Failed to process question: ${question.content.substring(0, 50)}...`,
  ...
  for (const example of starExamples) {
    try {
      ...
      // Since vectorDocument model doesn't exist, we'll skip creation entirely
      console.log(`⏭️  Skipping creation (no vectorDocument model): ${example.content.substring(0, 50)}...`);
      skipped++;
    } catch (error) {
      console.error(
        `❌ Failed to process STAR example: ${example.content.substring(0, 50)}...`,
        error instanceof Error ? error.message : String(error)
      );
      failed++;
    }
